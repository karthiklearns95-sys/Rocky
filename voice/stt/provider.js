...
import eventBus from '#voice/services/eventBus.js';
import normalizeCommand from '#voice/stt/normalizer.js';
import tts from '#voice/tts/textToSpeech.js'; // Fallback for repeating
...
export default class SpeechProvider {
  ...
  setupListeners() {
    ...
    ipcMain.on('speech-result', async (event, data) => {
      ...
      const { default: brain } = await import('#voice/core/index.js');
