 ...
 * uiaManager.js
 * 
 * Bridges Node.js with Microsoft UI Automation (UIA) via a generated PowerShell script.
 */
class UIAManager {
  constructor() {
    this.scriptPath = path.join(__dirname, 'uiaController.ps1');
...
`;
  ...
  async runCommand(action, query = "", roleHint = "", value = "", signal = null) {
    ...
    const cmd = `powershell -NoProfile -ExecutionPolicy Bypass -File "${this.scriptPath}" -Action "${action}" -Query "${safeQuery}" -RoleHint "${roleHint}" -Value "${safeValue}"`;
    
    return new Promise((resolve) => {
      const child = exec(cmd, { maxBuffer: 1024 * 1024 * 5, signal }, (error, stdout) => {
        if (error) {
          if (error.name === 'AbortError') {
            return resolve({ success: false, error: 'Aborted', isAbort: true });
          }
          if (!stdout) return resolve({ success: false, error: error.message });
        }
        try {
          const res = JSON.parse(stdout.trim());
          resolve(res);
        } catch {
          resolve({ success: false, error: "Failed to parse UIA output", raw: stdout });
        }
      });
    });
  }
